...
class OWFace(OWWidget):
    ...
    def _construct_output_data_table(embedded_images, embeddings):
        ...
        new_metas = [StringVariable('match'), StringVariable('match image')]
        
        X = embedded_images.X
        Y = np.array([100*float(e[1]) for e in embeddings]).reshape(-1, 1)
        M = np.array([[e[0], e[2]] for e in embeddings])
